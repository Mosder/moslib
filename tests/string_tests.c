...
TEST(creating) {
    ...
    test_assert(
        ss4.data == str && ss4.len == sizeof(HELLO_WORLD_LIT) - 1, "SS_LIT created wrong slice"
    );
...
TEST(comparing) {
    ...
    test_assert(
        ss_eq(ss, ss3) == 0, "slice is equal when the pointer is the same but length differs"
    );

    StringSlice ss4 = ss_from_str("hello");
    StringSlice ss5 = ss_from_str("world");
    test_assert(
        ss_eq(ss4, ss5) == 0, "slice is equal when the length is the same but pointer differs"
    );
...
TEST(prefix_suffix) {
    ...
    test_assert(
        ss_starts_with(ss, ss_from_str("pre")), "slice doesn't start with its slice prefix"
    );
    ...
    test_assert(
        ss_starts_with(ss2, ss_from_str("")), "starts_with breaks for empty slice and prefix"
    );
...
TEST(splitting) {
    ...
    test_assert(
        ss_eq_str(ss2, "Hello") && ss_eq_str(ss1, " World!"), "ss_split didn't split proprely"
    );
    ...
    test_assert(
        ss_eq_str(ss2, "") && ss_eq_str(ss1, "hiii"), "ss_split_str fails empty str edge case"
    );

    ss2 = ss_split_ss(&ss1, SS_LIT(""));
    test_assert(
        ss_eq_str(ss2, "") && ss_eq_str(ss1, "hiii"), "ss_split_ss fails empty ss edge case"
    );

    ss2 = ss_split(&ss1, '.');
    test_assert(
        ss_eq_str(ss2, "hiii") && ss_eq_str(ss1, ""), "ss_split fails not found delim edge case"
    );

    ss1 = ss_split_str(&ss2, "..");
    test_assert(
        ss_eq_str(ss1, "hiii") && ss_eq_str(ss2, ""), "ss_split_str fails not found delim edge case"
    );

    ss2 = ss_split_ss(&ss1, SS_LIT("..."));
    test_assert(
        ss_eq_str(ss2, "hiii") && ss_eq_str(ss1, ""), "ss_split_ss fails not found delim edge case"
    );
    ...
    test_assert(
        ss_eq_str(ss_left, "[left]") && ss_eq_str(ss_middle, "[middle][right]"),
        "ss_split_left didn't split properly"
    );

    StringSlice ss_right = ss_split_right(&ss_middle, 7);
    test_assert(
        ss_eq_str(ss_right, "[right]") && ss_eq_str(ss_middle, "[middle]"),
        "ss_split_right didn't split properly"
    );

    ss_left = ss_split_left(&ss_middle, 20);
    test_assert(
        ss_eq_str(ss_left, "[middle]") && ss_eq_str(ss_middle, ""),
        "ss_split_left didn't split properly (n > len)"
    );

    ss_right = ss_split_right(&ss_middle, 500);
    test_assert(
        ss_eq_str(ss_right, "") && ss_eq_str(ss_middle, ""),
        "ss_split_right didn't split properly (n > len)"
    );
}
...
TEST(finding) {
    ...
    test_assert(
        ss_eq_str(ss_find(ss, ss_from_str("FINDME")), "FINDME text"),
        "find didn't find a part of the slice"
    );
    test_assert(
        ss_eq_str(ss_find_str(ss, "FINDME"), "FINDME text"),
        "find_str didn't find a part of the slice"
    );

    test_assert(
        ss_eq(ss_find(ss, ss_from_str("")), ss), "find didn't return whole slice for empty target"
    );
    test_assert(
        ss_eq(ss_find_str(ss, ""), ss), "find_str didn't return whole slice for empty target"
    );

    test_assert(
        ss_eq_str(ss_find(ss, ss_from_str("abcd")), ""),
        "find didn't return empty slice for not found target"
    );
    test_assert(
        ss_eq_str(ss_find_str(ss, "abcd"), ""),
        "find_str didn't return empty slice for not found target"
    );
}
